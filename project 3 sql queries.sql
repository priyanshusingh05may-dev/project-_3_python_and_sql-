...
FROM (
    SELECT YEAR(order_purchase_timestamp) AS year, c.customer_unique_id, SUM(p.payment_value) AS total_spent,
           RANK() OVER(PARTITION BY YEAR(order_purchase_timestamp) ORDER BY SUM(p.payment_value) DESC) AS customer_rank
    FROM orders o
    JOIN customers c ON o.customer_id = c.customer_id
    JOIN payments p ON o.order_id = p.order_id
